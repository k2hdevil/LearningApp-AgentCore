...
+---------------------------------------------------------------------+
```
*기존 생성형 AI(단일 턴)와 에이전틱 AI(다단계 루프) 아키텍처 비교*
...
+--------------+
```
*에이전틱 루프의 반복 실행 흐름: 인식→추론→행동→관찰→목표 확인*
...
agentcore teardown
```
*AgentCore CLI의 에이전트 전체 수명주기 관리 명령어*
...
+---------------------------------------------------------------------+
```
*AgentCore 서비스를 활용한 고객 서비스 에이전트 전체 아키텍처*
...
```
    ...
    +--------+ +--------+ +--------+
```
*오케스트레이터가 전문화된 워커 에이전트에 작업을 위임하는 패턴*
...
+--------------------+Protocol +--------------------+
```
*에이전트(MCP 클라이언트)와 MCP 서버 간의 표준화된 통신*
...
```
*AgentCore GA 리전 가용성 (2026년 5월 기준)*
