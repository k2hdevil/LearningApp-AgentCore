...
+----------------------------------------------------------+
   ...
   +---------+              +---------------------------+
```
*AgentCore Observability의 서비스 위치: 모든 모델/프레임워크를 지원하는 OTEL 기반 관찰성 계층*
...
+-------------------------------------------------------------+
```
*Observability 계층 구조: 세션 > 트레이스 > 스팬 > 서브 스팬*
...
```
*쇼핑몰 고객 지원 에이전트 시나리오: 세션 > 트레이스 > 스팬 > 서브 스팬의 계층적 관찰성 구조*
...
+---------------------------------------------------------------+
```
*Observability 4단계 드릴다운 뷰 계층 구조*
...
+----------+     | Runtime         |     +----------+
              ...
              +-------+------+ +-------+------+
                  ...
                  +--------------------------+
```
*AgentCore Evaluations 아키텍처: 트레이스 수집 → LLM 심판 평가 → 결과 확인*
...
+--------------------------+     +-------------------+     | Input: Prompt template +     |
                                                           ...
                                                           +------------------------------+
```
*LLM-as-a-Judge 평가 흐름: 에이전트 트레이스 수집 → 통일 형식 변환 → 심판 모델 평가 → 대시보드 표시*
...
+----------------------+     +------------+-------------+
                             ...
                             +--------------------------+
```
*코드 기반 평가 흐름: 트레이스 수집 → Lambda 함수로 프로그래밍적 평가 → 대시보드 표시*
...
```
*Agent Performance Loop: 관찰→평가→최적화→배포의 지속적 폐쇄 루프*
...
+------------------+  +------------------+  +----------------------+
```
*Cross-Account Monitoring 아키텍처: 여러 소스 계정의 에이전트를 중앙에서 통합 모니터링*
...
```
*Evaluations를 CI/CD 파이프라인에 통합하는 패턴: 빌드→배치 평가→시뮬레이션→배포 승인*
