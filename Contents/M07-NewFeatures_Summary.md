...
└───────────────────────────────┬───────────────────────────────────┘
                                ...
                                ▼
┌───────────────────────────────────────────────────────────────────┐
│           AgentCore 자동 관리 (격리된 microVM)                      │
│                                                                   │
│  • 에이전틱 루프: 추론 → 도구 선택 → 실행 → 응답 스트리밍          │
│  • 세션 격리: 각 세션 독립 microVM                                 │
│  • Observability: 모델/도구/메모리/셸 모든 스팬 자동 기록          │
│  • 비용 제어: 토큰 예산, 반복 제한, 타임아웃                       │
│  • 파일시스템: 영구 세션 스토리지 + S3/EFS 마운트                  │
└───────────────────────────────────────────────────────────────────┘
```
...
┌──────────────────────────┐     ┌──────────────────────────────────┐
│ 에이전트가 유료 API       │────▶│ HTTP 402 Payment Required        │
│ 호출 시도                 │     │ 응답 수신                         │
└──────────────────────────┘     └────────────────┬─────────────────┘
                                                  │
                                                  ▼
...
│                      AgentCore Payments                           │
│                                                                  │
│  1. 지갑 인증 (Wallet Authentication)                            │
│     • Coinbase: 스테이블코인 (USDC)                              │
│     • Stripe/Privy: 법정화폐 (USD)                               │
│                         │                                        │
│                         ▼                                        │
│  2. 정책 확인 (Spending Governance)                              │
│     • 예산 한도 확인                                             │
│     • 승인 정책 평가                                             │
│     • 거래 금액 검증                                             │
│                         │                                        │
...
│  │                                                             │  │
│  │  ┌──────────────┐  ┌──────────┐  ┌───────────────────────┐ │  │
│  │  │ Console UI   │  │   API    │  │ MCP 서버 (IDE 쿼리)    │ │  │
│  │  │ (웹 대시보드) │  │ (boto3)  │  │ (Kiro, Cursor 등)    │ │  │
│  │  └──────────────┘  └──────────┘  └───────────────────────┘ │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                                                   │
│  ┌─────────────────────────────────────────────────────────────┐  │
...
```
M01 기초 ──▶ M02 Runtime ──▶ M03 보안 ──▶ M04 도구/Gateway ──▶ M05 메모리 ──▶ M06 관찰성/평가 ──▶ M07 마무리
                  │                            │                                      │
                  ▼                            ▼                                      ▼
      ┌────────────────────────┐  ┌─────────────────────────────┐  ┌──────────────────────────────────┐
      │🆕 Harness + CLI 소개   │  │🆕 Payments + Registry 소개   │  │🆕 Performance Loop 소개          │
      │    (15-20분)           │  │    (15-20분)                 │  │    (15-20분)                     │
      └────────────────────────┘  └─────────────────────────────┘  └──────────────────────────────────┘
```
