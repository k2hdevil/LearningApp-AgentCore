...
```
*도구 통합 시 어떤 방식을 선택할지 결정하는 의사결정 트리 (Tool integration decision tree)*
...
+--------------------------+
```
*사용자 쿼리가 에이전트와 LLM을 거쳐 헤드리스 브라우저에서 실행되는 흐름*
...
+--------------------------------------------------------------------------+
```
*AgentCore Browser 아키텍처: 사용자 코드 라이브러리와 AWS 관리형 MicroVM 인프라 구성*
...
+--------------+    +----+-----+    +----------------------------------+
              ...
              +---------------------------------+
```
*Code Interpreter의 작동 흐름: 사용자 쿼리에서 격리된 세션 생성 및 결과 반환까지*
...
```
*코드 실행 샌드박스 선택 가이드 (Code execution sandbox selection guide)*
...
+------------------+     +------------------+
```
*MCP 아키텍처: 호스트 프로세스 내 클라이언트가 원격 MCP 서버와 통신하는 구조*
...
+----------------------------------------------------------------------+
```
*로컬 호스팅 vs 원격 호스팅: 에이전트와 MCP 서버의 배치 방식 비교*
...
```
*AgentCore Gateway 아키텍처: 에이전트가 MCP 프로토콜을 통해 다양한 대상에 접근*
...
+----------------------------------------------------------------------+
```
*도구 수에 따른 검색 전략: 소규모는 전체 나열, 대규모는 시맨틱 검색으로 필터링*
...
```
*전체 나열 vs 시맨틱 검색의 차이: 컨텍스트 초과 위험 방지*
...
```
*Lambda 대상 호출 흐름: Gateway가 MCP 도구 호출을 Lambda 이벤트로 변환*
...
+--------------+                                       |
                                ...
                                +------------------+  +--------------------+
```
*OpenAPI / MCP 서버 대상 통합 시 인바운드·아웃바운드 인증 흐름*
...
+------------------------+    |                                          |
  ...
  +--------------------+     +----------+----------------+--------------+
                              ...
                              +------------------+  +--------------------+
```
*AgentCore Policy 아키텍처: 정책 평가를 통한 허용/거부 결정 흐름*
...
+-------------------------------------------------------------------+
```
*AWS Agent Registry: 도구, 에이전트, 스킬 등을 중앙에서 관리하는 카탈로그*
